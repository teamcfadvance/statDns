<cfcomponent hint="Consumes the statdns.com API">
	...
	<cffunction name="isIPv6" access="private" hint="Returns whether or not the passed value is a valid IPv6 address" output="no" returntype="boolean">
		...
		<cfscript>
			if (refind("(^\d{20}$)|(^((:[a-fA-F0-9]{1,4}){6}|::)ffff:(25[0-5]|2[0-4][0-9]|1[0-9][0-9]|[0-9]{1,2})(\.(25[0-5]|2[0-4][0-9]|1[0-9][0-9]|[0-9]{1,2})){3}$)|(^((:[a-fA-F0-9]{1,4}){6}|::)ffff(:[a-fA-F0-9]{1,4}){2}$)|(^([a-fA-F0-9]{1,4}) (:[a-fA-F0-9]{1,4}){7}$)|(^:(:[a-fA-F0-9]{1,4}(::)?){1,6}$)|(^((::)?[a-fA-F0-9]{1,4}:){1,6}:$)|(^::$)",arguments.ip)) {
				return true;
			}
			else {
				return false;
